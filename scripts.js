// script.js (updated with SEO-friendly path routing)
// Features:
...
// - history.pushState with SEO-friendly path (/Book/Chapter[/Verse], /search/keyword)
...
function showVerse(push = true) {
  ...
  if (push) {
    const path = `/${encodeURIComponent(currentBook)}/${currentChapter}/${currentVerse}`;
...
function showChapter(push = true) {
  ...
  if (push) {
    const path = `/${encodeURIComponent(currentBook)}/${currentChapter}`;
...
// === Attach delegated click handler to #searchResults ===
